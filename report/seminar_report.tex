...
\begin{abstract}
\textbf{Abstract (Fynn).} Long Short Term Memory is a commonly used Recurrent Neural Network Architecture. This paper will give a short overview of the foundational concepts leading up to the invention of the architecture. Then the architecture will be explained, before formalizing. Afterwards an example implementation using PyTorch will be showcased, and tested on a simple sequence. Next limitations will be discussed, leading to variations of the architecture, such as the common addition of a forget gate, as well as a comparison to alternative architectures such as Transformers. Afterwards some applications of the architecture will be highlighted. Finally the main results are summed up and a brief outlook for the future of the LSTM architecture is given.
\end{abstract}
