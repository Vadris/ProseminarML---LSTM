...
\captionof{figure}{Conceptual visualization of a single perceptron, showing weighted inputs, a bias term, and an activation function.}
...
\begin{center}
  \begin{tikzpicture}[>=latex,
    ...
    \path
      ...
      (h);
  \end{tikzpicture}
  \captionof{figure}{A simple RNN cell with one hidden layer. The hidden state $h_t$ is computed from the current input $x_t$ and the previous hidden state $h_{t-1}$, and then mapped to the output $y_t$.}
  \label{fig:rnn}
\end{center}
...
\begin{center}
  ...
  \end{tikzpicture}
  \captionof{figure}{An Elman RNN unrolled over $T=3$ time steps. Each copy of the RNN cell shares the same weights but processes a different input $x_t$, producing hidden states $h_t$ and outputs $y_t$.}\label{fig:rnn_unroll}
\end{center}
